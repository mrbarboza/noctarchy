...
- `bin/capture-screenshot` (bound Mod+Print/Mod+Shift+Print in `config/niri/config.kdl`'s `binds{}`) ports Omarchy's screenshot-with-clipboard-and-notification flow to niri: `slurp` for region, `niri msg --json focused-output | jq '.logical'` for the fullscreen geometry (niri has no `hyprctl monitors` equivalent), then `grim -g`/`wl-copy`/best-effort `notify-send -A`. It intentionally skips Omarchy's window/smart snap-to-window modes (need Hyprland IPC) and its `hyprctl` hardware-cursor toggle (no niri equivalent) - see any follow-up task for those.
